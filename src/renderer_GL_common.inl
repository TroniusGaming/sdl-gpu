...
static void forceChangeBlendMode(GPU_Renderer* renderer, GPU_BlendMode mode)
{
	...
	if (renderer->enabled_features & GPU_FEATURE_BLEND_EQUATIONS)
	{
		...
		{
			GPU_PushErrorCode("(SDL_gpu internal)", GPU_ERROR_BACKEND_ERROR,
			                  "Could not set blend equation because GPU_FEATURE_BLEND_EQUATIONS_SEPARATE is not supported.");
		}
	}
	else
	{
...
{
	GPU_CONTEXT_DATA* cdata = (GPU_CONTEXT_DATA*)renderer->current_context_target->context->data;
	if (cdata->last_blend_mode.source_color == mode.source_color && cdata->last_blend_mode.dest_color == mode.dest_color &&
	    cdata->last_blend_mode.source_alpha == mode.source_alpha && cdata->last_blend_mode.dest_alpha == mode.dest_alpha &&
	    cdata->last_blend_mode.color_equation == mode.color_equation && cdata->last_blend_mode.alpha_equation == mode.alpha_equation)
		return;
...
static void changeViewport(GPU_Target* target)
{
	...
	if (cdata->last_viewport.x == target->viewport.x && cdata->last_viewport.y == target->viewport.y && cdata->last_viewport.w == target->viewport.w &&
...
static GPU_bool equal_cameras(GPU_Camera a, GPU_Camera b)
{
	return (a.x == b.x && a.y == b.y && a.z == b.z && a.angle == b.angle && a.zoom_x == b.zoom_x && a.zoom_y == b.zoom_y &&
	        a.use_centered_origin == b.use_centered_origin);
}
...
// ARB_compatibility brings support for this to GL 3.1, but glGetStringi() via GLEW has chicken and egg problems.
	...
	if (!(GPU_flags & GPU_INIT_DISABLE_AUTO_VIRTUAL_RESOLUTION) && w != 0 && h != 0 &&
...
static GPU_Target* CreateTargetFromWindow(GPU_Renderer* renderer, Uint32 windowID, GPU_Target* target)
{
	...
	{
		GPU_PushErrorCode("GPU_CreateTargetFromWindow", GPU_ERROR_BACKEND_ERROR,
		                  "Renderer major version (%d) is incompatible with the available OpenGL runtime library version (%d).", renderer->requested_id.major_version,
		                  renderer->id.major_version);
		target->context->failed = GPU_TRUE;
		return NULL;
	}
...
static GPU_Image* CreateUninitializedImage(GPU_Renderer* renderer, Uint16 w, Uint16 h, GPU_FormatEnum format)
{
	...
	switch (format)
	{
		...
		case GPU_FORMAT_YCbCr422:
			...
			bytes_per_pixel = 1;
			break;
		default: GPU_PushErrorCode("GPU_CreateUninitializedImage", GPU_ERROR_DATA_ERROR, "Unsupported image format (0x%x)", format); return NULL;
	}
...
static GPU_Image* CreateImageUsingTexture(GPU_Renderer* renderer, GPU_TextureHandle handle, GPU_bool take_ownership)
{
	...
	{
		GPU_PushErrorCode("GPU_CreateImageUsingTexture", GPU_ERROR_UNSUPPORTED_FUNCTION,
		                  "Renderer %s's runtime version on this device (3.0) does not support this function", renderer->id.name);
		return NULL;
	}
#endif
	...
	switch (gl_format)
	{
		...
		case GL_RG:
			...
			break;
#endif
		default: GPU_PushErrorCode("GPU_CreateImageUsingTexture", GPU_ERROR_DATA_ERROR, "Unsupported GL image format (0x%x)", gl_format); return NULL;
	}
	...
	switch (min_filter)
	{
		case GL_NEAREST: filter_mode = GPU_FILTER_NEAREST; break;
		case GL_LINEAR:
		case GL_LINEAR_MIPMAP_NEAREST: filter_mode = GPU_FILTER_LINEAR; break;
		case GL_LINEAR_MIPMAP_LINEAR: filter_mode = GPU_FILTER_LINEAR_MIPMAP; break;
		default:
	...
	switch (wrap_s)
	{
		case GL_CLAMP_TO_EDGE: wrap_x = GPU_WRAP_NONE; break;
		case GL_REPEAT: wrap_x = GPU_WRAP_REPEAT; break;
		case GL_MIRRORED_REPEAT: wrap_x = GPU_WRAP_MIRRORED; break;
		default:
	...
	switch (wrap_t)
	{
		case GL_CLAMP_TO_EDGE: wrap_y = GPU_WRAP_NONE; break;
		case GL_REPEAT: wrap_y = GPU_WRAP_REPEAT; break;
		case GL_MIRRORED_REPEAT: wrap_y = GPU_WRAP_MIRRORED; break;
		default:
...
static GPU_bool SaveImage(GPU_Renderer* renderer, GPU_Image* image, const char* filename, GPU_FileFormatEnum format)
{
	...
	if (image == NULL || filename == NULL || image->texture_w < 1 || image->texture_h < 1 || image->bytes_per_pixel < 1 || image->bytes_per_pixel > 4)
	{
		return GPU_FALSE;
	}
...
static GPU_Image* gpu_copy_image_pixels_only(GPU_Renderer* renderer, GPU_Image* image)
{
	...
	switch (image->format)
	{
		...
		case GPU_FORMAT_RG:
			...
			break;
		default: GPU_PushErrorCode("GPU_CopyImage", GPU_ERROR_BACKEND_ERROR, "Could not copy the given image format."); break;
	}
...
static void FreeImage(GPU_Renderer* renderer, GPU_Image* image)
{
	...
	if (image->target != NULL)
	{
		...
		// Freeing it will decrement the refcount.  If this is the only increment, it will be freed.  This means GPU_LoadTarget() needs to be paired with
		// GPU_FreeTarget().
		target->refcount++;
...
static GPU_Target* GetTarget(GPU_Renderer* renderer, GPU_Image* image)
{
	...
	if (status != GL_FRAMEBUFFER_COMPLETE)
	{
		GPU_PushErrorCode("GPU_GetTarget", GPU_ERROR_DATA_ERROR,
		                  "Framebuffer incomplete with status: 0x%x.  Format 0x%x for framebuffers might not be supported on this hardware.", status,
		                  ((GPU_IMAGE_DATA*)image->data)->format);
		return NULL;
	}
...
static void BlitTransformX(GPU_Renderer* renderer, GPU_Image* image, GPU_Rect* src_rect, GPU_Target* target, float x, float y, float pivot_x, float pivot_y,
...
{
	if (type == GPU_TYPE_DOUBLE)
		return sizeof(double);
	if (type == GPU_TYPE_FLOAT)
		return sizeof(float);
	if (type == GPU_TYPE_INT)
		return sizeof(int);
	if (type == GPU_TYPE_UNSIGNED_INT)
		return sizeof(unsigned int);
	if (type == GPU_TYPE_SHORT)
		return sizeof(short);
	if (type == GPU_TYPE_UNSIGNED_SHORT)
		return sizeof(unsigned short);
	if (type == GPU_TYPE_BYTE)
		return sizeof(char);
	if (type == GPU_TYPE_UNSIGNED_BYTE)
...
static void upload_attribute_data(GPU_CONTEXT_DATA* cdata, int num_vertices)
{
	...
	for (i = 0; i < 16; i++)
	{
		...
		if (a->attribute.values != NULL && a->attribute.location >= 0 && a->num_values > 0)
		{
			...
			glVertexAttribPointer(a->attribute.location, a->attribute.format.num_elems_per_value, a->attribute.format.type, a->attribute.format.normalize,
			                      a->per_vertex_storage_stride_bytes, (void*)(intptr_t)a->per_vertex_storage_offset_bytes);
...
static void PrimitiveBatchV(GPU_Renderer* renderer, GPU_Image* image, GPU_Target* target, GPU_PrimitiveEnum primitive_type, unsigned short num_vertices, void* values,
...
static void SetImageFilter(GPU_Renderer* renderer, GPU_Image* image, GPU_FilterEnum filter)
{
	...
	switch (filter)
	{
		...
		case GPU_FILTER_LINEAR_MIPMAP:
			...
			magFilter = GL_LINEAR;
			break;
		default: GPU_PushErrorCode("GPU_SetImageFilter", GPU_ERROR_USER_ERROR, "Unsupported value for filter (0x%x)", filter); return;
	}
...
static void SetWrapMode(GPU_Renderer* renderer, GPU_Image* image, GPU_WrapEnum wrap_mode_x, GPU_WrapEnum wrap_mode_y)
{
	...
	switch (wrap_mode_x)
	{
		case GPU_WRAP_NONE: wrap_x = GL_CLAMP_TO_EDGE; break;
		case GPU_WRAP_REPEAT: wrap_x = GL_REPEAT; break;
		case GPU_WRAP_MIRRORED:
			...
			break;
		default: GPU_PushErrorCode("GPU_SetWrapMode", GPU_ERROR_USER_ERROR, "Unsupported value for wrap_mode_x (0x%x)", wrap_mode_x); return;
	}

	switch (wrap_mode_y)
	{
		case GPU_WRAP_NONE: wrap_y = GL_CLAMP_TO_EDGE; break;
		case GPU_WRAP_REPEAT: wrap_y = GL_REPEAT; break;
		case GPU_WRAP_MIRRORED:
			if (renderer->enabled_features & GPU_FEATURE_WRAP_REPEAT_MIRRORED)
			...
			break;
		default: GPU_PushErrorCode("GPU_SetWrapMode", GPU_ERROR_USER_ERROR, "Unsupported value for wrap_mode_y (0x%x)", wrap_mode_y); return;
	}
...
static void DoPartialFlush(GPU_Renderer* renderer, GPU_Target* dest, GPU_Context* context, unsigned short num_vertices, float* blit_buffer, unsigned int num_indices,
                           unsigned short* index_buffer)
{
...
// Update the vertex array object's buffers
		...
		{
			glEnableVertexAttribArray(context->current_shader_block.texcoord_loc);
			glVertexAttribPointer(context->current_shader_block.texcoord_loc, 2, GL_FLOAT, GL_FALSE, GPU_BLIT_BUFFER_STRIDE,
			                      (void*)(GPU_BLIT_BUFFER_TEX_COORD_OFFSET * sizeof(float)));
		}
		if (context->current_shader_block.color_loc >= 0)
		{
			glEnableVertexAttribArray(context->current_shader_block.color_loc);
			glVertexAttribPointer(context->current_shader_block.color_loc, 4, GL_FLOAT, GL_FALSE, GPU_BLIT_BUFFER_STRIDE,
			                      (void*)(GPU_BLIT_BUFFER_COLOR_OFFSET * sizeof(float)));
		}
...
static void DoUntexturedFlush(GPU_Renderer* renderer, GPU_Target* dest, GPU_Context* context, unsigned short num_vertices, float* blit_buffer, unsigned int num_indices,
                              unsigned short* index_buffer)
{
	GPU_CONTEXT_DATA* cdata = (GPU_CONTEXT_DATA*)context->data;
	(void)renderer;
...
static Uint32 compile_shader_source(GPU_ShaderEnum shader_type, const char* shader_source)
{
	...
	switch (shader_type)
	{
		case GPU_VERTEX_SHADER: shader_object = glCreateShader(GL_VERTEX_SHADER); break;
		case GPU_FRAGMENT_SHADER: shader_object = glCreateShader(GL_FRAGMENT_SHADER); break;
...
static void SetUniformiv(GPU_Renderer* renderer, int location, int num_elements_per_value, int num_values, int* values)
{
	...
	{
		case 1: glUniform1iv(location, num_values, values); break;
		case 2: glUniform2iv(location, num_values, values); break;
		case 3: glUniform3iv(location, num_values, values); break;
		case 4: glUniform4iv(location, num_values, values); break;
	}
#endif
}
...
static void SetUniformuiv(GPU_Renderer* renderer, int location, int num_elements_per_value, int num_values, unsigned int* values)
{
	...
	switch (num_elements_per_value)
	{
		case 1: glUniform1iv(location, num_values, (int*)values); break;
		case 2: glUniform2iv(location, num_values, (int*)values); break;
		case 3: glUniform3iv(location, num_values, (int*)values); break;
		case 4: glUniform4iv(location, num_values, (int*)values); break;
	}
#else
	switch (num_elements_per_value)
	{
		case 1: glUniform1uiv(location, num_values, values); break;
		case 2: glUniform2uiv(location, num_values, values); break;
		case 3: glUniform3uiv(location, num_values, values); break;
		case 4: glUniform4uiv(location, num_values, values); break;
	}
#endif
#endif
}
...
static void SetUniformfv(GPU_Renderer* renderer, int location, int num_elements_per_value, int num_values, float* values)
{
	...
	switch (num_elements_per_value)
	{
		case 1: glUniform1fv(location, num_values, values); break;
		case 2: glUniform2fv(location, num_values, values); break;
		case 3: glUniform3fv(location, num_values, values); break;
		case 4: glUniform4fv(location, num_values, values); break;
	}
#endif
}
...
static void SetAttributefv(GPU_Renderer* renderer, int location, int num_elements, float* value)
{
	...
	switch (num_elements)
	{
		case 1: glVertexAttrib1f(location, value[0]); break;
		case 2: glVertexAttrib2f(location, value[0], value[1]); break;
		case 3: glVertexAttrib3f(location, value[0], value[1], value[2]); break;
		case 4: glVertexAttrib4f(location, value[0], value[1], value[2], value[3]); break;
	}
...
static void SetAttributeiv(GPU_Renderer* renderer, int location, int num_elements, int* value)
{
	...
	switch (num_elements)
	{
		case 1: glVertexAttribI1i(location, value[0]); break;
		case 2: glVertexAttribI2i(location, value[0], value[1]); break;
		case 3: glVertexAttribI3i(location, value[0], value[1], value[2]); break;
		case 4: glVertexAttribI4i(location, value[0], value[1], value[2], value[3]); break;
	}
...
static void SetAttributeuiv(GPU_Renderer* renderer, int location, int num_elements, unsigned int* value)
{
	...
	switch (num_elements)
	{
		case 1: glVertexAttribI1ui(location, value[0]); break;
		case 2: glVertexAttribI2ui(location, value[0], value[1]); break;
		case 3: glVertexAttribI3ui(location, value[0], value[1], value[2]); break;
		case 4: glVertexAttribI4ui(location, value[0], value[1], value[2], value[3]); break;
	}
